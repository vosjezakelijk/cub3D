...
/*   Updated: 2021/06/09 14:07:42 by hsamatha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	key_wa(t_param *param, double move)
{
	if (param->act.a == 1)
	{
		if (param->map.map[(int)(param->act.pos.y)][(int)(param->act.pos.x + param->act.plane.x * move)] == '0') //collision, if no walls and sprites on its way - can move
			param->act.pos.x += param->act.plane.x * move;
		if (param->map.map[(int)(param->act.pos.y + param->act.plane.y * move)][(int)(param->act.pos.x)] == '0') //collision
			param->act.pos.y += param->act.plane.y * move;
	}
    if (param->act.w == 1)
	{
		if (param->map.map[(int)(param->act.pos.y)][(int)(param->act.pos.x + param->act.dir.x * move)] == '0')
			param->act.pos.x += param->act.dir.x * move;
		if (param->map.map[(int)(param->act.pos.y + param->act.dir.y * move)][(int)(param->act.pos.x)] == '0')
...
static void	key_sd(t_param *param, double move)
{
	...
	if (param->act.d == 1)
	{
		if (param->map.map[(int)(param->act.pos.y)][(int)(param->act.pos.x - param->act.plane.x * move)] == '0')
			param->act.pos.x -= param->act.plane.x * move;
		if (param->map.map[(int)(param->act.pos.y - param->act.plane.y * move)][(int)(param->act.pos.x)] == '0')
			param->act.pos.y -= param->act.plane.y * move;
	}
}
...
int	movement(t_param *param)
{
	...
	move = 0.09; //chanegable
	rot = 0.09; //changeable
	if (param->act.left == 1 || param->act.right == 1) //turn 
	{	
...
void	render_sprites(t_param *param)
{
	...
	y = 0;
	count_spr = 0;
	sort_sprites(param, count_spr); //to define the distance
	count_spr = 0;
...
void	render(t_param *param, int x, int y) 
{
	...
	x = 0;
	while (x < param->map.scr_w) //render floor and ceiling
	{
	...
	x = 0;
	while (x < param->map.scr_w) //render walls
	{
		raycast(param, x);
		draw_pixel(param, x);
		x++;
